...
det = r"C:\Users\sam.lu\.paddleocr\whl\det\ch\ch_PP-OCRv4_det_infer" # change detection model path of your own 
rec = r"C:\Users\sam.lu\.paddleocr\whl\rec\ch\ch_PP-OCRv4_rec_infer" # change recognition model path of your own 
cls = r"C:\Users\sam.lu\.paddleocr\whl\cls\ch_ppocr_mobile_v2.0_cls_infer" # change classification model path of your own 
...
want = ['Max', 'Min', 'Rise', 'Fall', 'Freq', 'Pos Wid', 'Neg Wid', 'Dely',
        'Max*', 'Min*', 'Rise*', 'Fall*', 'Freq*', 'Pos Wid*', 'Neg Wid*', 'Dely*',
        'PosWid', 'NegWid','PosWid*', 'NegWid*',
        'Pk-Pk', 'High', 'Low', 'Period', '+DtyCyc', '-DtyCyc', '+Ovrsht', '-Ovrsht',
        'Pk-Pk*', 'High*', 'Low*', 'Period*', '+DtyCyc*', '-DtyCyc*', '+Ovrsht*', '-Ovrsht*']
...
def OCR(imagepath):
    ...
    for i, res in enumerate(result[0]):
        print(res)
        if res[1][0] in want and res[0][0][0] <= 70.0:
            dicts[res[1][0]] = result[0][i+1][1][0]
...
def Create_New_Excel(root):
    ...
    newwb.save(f'{root}\\Project_Signal-Integrity(I2C)_EVT_Test_Report_Rev.A0.xlsx') # name the excel "Project_Signal-Integrity_EVT_Test_Report_Rev.A0.xlsx"
...
def Get_Path():
    ...
    path = os.path.join(root, 'Result_SI', 'I2C')
    return root, path
...
def Result_1(ws, row, name, Probe, result1, spec1, result2, spec2, column, data, misslist, needloc=False, ):
    ...
    if needloc:
        return row,f'={locname}', f'={locprob}', misslist
...
def UnitTransform(dict):
    ...
    v = ["Max", "Max*"]
    mv = ["Min", "Min*"]
    ns = ["Rise", "Fall", "Dely", "Rise*", "Fall*", "Dely*"]
    us = ["Pos Wid", "Neg Wid", "PosWid", "NegWid", "Pos Wid*", "Neg Wid*", "PosWid*", "NegWid*"]
    khz = ["Freq"]
...
def main():
    misslist = [] # store the wrong cell
    root, path = Get_Path() # root = path of project result
    os.chdir(root)
    Create_New_Excel(root)
    wb = opxl.load_workbook(os.path.join(root,'Project_Signal-Integrity(I2C)_EVT_Test_Report_Rev.A0.xlsx'))
    ws = wb.active
    ExcelFormat(wb)
    row = 52
    row = VDDVol(ws, row)
    ...
    for dir in dirs:
        ...
        for file in files:
            if file == "CLK.jpg" :
                CLKdict = OCR(os.path.join(dir, file))
            elif file == "DATA.jpg" :
                DATAdict = OCR(os.path.join(dir, file))

        UnitTransform(CLKdict)
        UnitTransform(DATAdict)


        clk_max = CLKdict["Max*"] if "Max*" in CLKdict else (CLKdict["Max"] if "Max" in CLKdict else None)
        data_max = DATAdict["Max*"] if "Max*" in DATAdict else (DATAdict["Max"] if "Max" in DATAdict else None)
        clk_min = CLKdict["Min*"] if "Min*" in CLKdict else (CLKdict["Min"] if "Min" in CLKdict else None)
        data_min = DATAdict["Min*"] if "Min*" in DATAdict else (DATAdict["Min"] if "Min" in DATAdict else None)
        row = VolatageHeader(ws, row)
        row1 = row
        row, locname1, locprob1, misslist = Result_1(ws, row, "CLK", f'PinXX of {probname}', clk_max, "0.7*VDD ~ VDD+0.5", clk_min, "-0.6 ~ 0.3*VDD", "H", [2, 0, 2, 0, 0], misslist, True)
        row, locname2, locprob2, misslist = Result_1(ws, row, "DATA", f'PinYY of {probname}', data_max, "0.7*VDD ~ VDD+0.5", data_min, "-0.6 ~ 0.3*VDD", "H", [2, 0, 2, 0, 0], misslist, True)   
        row = NumOfTests(row1, row, ws)

        clk_rise = CLKdict["Rise*"] if "Rise*" in CLKdict else (CLKdict["Rise"] if "Rise" in CLKdict else None) 
        data_rise = DATAdict["Rise*"] if "Rise*" in DATAdict else (DATAdict["Rise"] if "Rise" in DATAdict else None)
        clk_fall = CLKdict["Fall*"] if "Fall*" in CLKdict else (CLKdict["Fall"] if "Fall" in CLKdict else None)
        data_fall = DATAdict["Fall*"] if "Fall*" in DATAdict else (DATAdict["Fall"] if "Fall" in DATAdict else None)
        row1 = row
        row = RiseFallTimeHeader(ws, row)
        row, misslist = Result_1(ws, row, locname1, locprob1, clk_rise, "1000", clk_fall, "300", "H", [2, 0, 2, 0, 0], misslist)
        row, misslist = Result_1(ws, row, locname2, locprob2, data_rise, "1000", data_fall, "300", "H", [2, 0, 2, 0, 0], misslist)   
        row = NumOfTests(row1, row, ws)

        clk_freq = CLKdict["Freq*"] if "Freq*" in CLKdict else (CLKdict["Freq"] if "Freq" in CLKdict else None) 
        clk_pos = CLKdict["Pos Wid*"] if "Pos Wid*" in CLKdict else (CLKdict["Pos Wid"] if "Pos Wid" in CLKdict else (CLKdict["PosWid"] if "PosWid" in CLKdict else None) ) 
        clk_neg = CLKdict["Neg Wid*"] if "Neg Wid*" in CLKdict else (CLKdict["Neg Wid"] if "Neg Wid" in CLKdict else (CLKdict["NegWid"] if "NegWid" in CLKdict else None))                    
        row1 = row
        row = CLKQualityHeader(ws, row)
        row, misslist = Result_2(ws, row, locname1, locprob1, clk_freq, "0~100", clk_pos, "4", clk_neg, "4.7", "J", [3, 0, 3, 0, 0], misslist)
        row = NumOfTests(row1, row, ws)

        clk_delay = CLKdict["Dely*"] if "Dely*" in CLKdict else (CLKdict["Dely"] if "Dely" in CLKdict else None)
        data_delay = DATAdict["Dely*"] if "Dely*" in DATAdict else (DATAdict["Dely"] if "Dely" in DATAdict else None)
        row1 = row
        row = SetupHoldTimmingHeader(ws, row)
        row, misslist = Result_1(ws, row, locname2, locprob2, clk_delay, "250", data_delay, "0", "H", [2, 0, 2, 0, 0], misslist)   
        row = NumOfTests(row1, row, ws)        

    wb.save(os.path.join(root, "Project_Signal-Integrity(I2C)_EVT_Test_Report_Rev.A0.xlsx"))

    print("total error cell: ", len(misslist))
    print(misslist)

    f = open(os.path.join(root, 'fail_to_rec(I2C).txt'), 'w')
