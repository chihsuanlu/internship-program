...
rec_table = "C:\\Users\\sam.lu\\.paddleocr\\whl\\rec\\ch\\ch_PP-OCRv3_rec_infer" # change recognition model path of your own 
cls = "C:\\Users\\sam.lu\\.paddleocr\\whl\\cls\\ch_ppocr_mobile_v2.0_cls_infer" # change classification model path of your own 

rec_dleta = "C:\\Users\\sam.lu\\.paddleocr\\whl\\rec\\ch\\ch_PP-OCRv4_rec_infer" # change recognition model path of your own 

# init OCR
ocr_table = PaddleOCR(
    lang="ch",
    det_model_dir=det,
    rec_model_dir=rec_table,
    cls_model_dir=cls,
)
...
ocr_dleta = PaddleOCR(
    lang="ch",
    det_model_dir=det,
    rec_model_dir=rec_dleta,
    cls_model_dir=cls,
)
'''

ocr_table = PaddleOCR(lang='ch') # 我是原code，第一次執行完把我註解
ocr_dleta = PaddleOCR(Alang='ch') # 我是原code，第一次執行完把我註解


want = ["Top", "Base", "Rise Time", "Fall Time", "Frequency", "High Time", "Low Time",
        " Top", " Base", " Rise Time", " Fall Time", " Frequency", " High Time", " Low Time",
        "Top ", "Base ", "Rise Time ", "Fall Time ", "Frequency ", "High Time ", "Low Time ",
        "Fal Time", " Fal Time", "Fal Time "]

def OCR_table(imagepath, dict1, dict2, ):
    print(f'\n{imagepath} is OCRing...')
    img = cv2.imread(imagepath)
    # OCR results
    result = ocr_table.ocr(img, cls=False)
    for i, res in enumerate(result[0]):
        print(res)
        if "vihvil" in imagepath and res[1][0] in want and res[0][0][0] < 230:
            if result[0][i+1][1][0] == "Ch 1" or result[0][i+1][1][0] == "ch 1" or\
               result[0][i+1][1][0] == " Ch 1" or result[0][i+1][1][0] == " ch 1" or\
               result[0][i+1][1][0] == "Ch 1 " or result[0][i+1][1][0] == "ch 1 ":
                dict1[res[1][0]] = result[0][i+2][1][0]
            elif result[0][i+1][1][0] == "Ch 2" or result[0][i+1][1][0] == "ch 2" or\
                 result[0][i+1][1][0] == " Ch 2" or result[0][i+1][1][0] == " ch 2" or\
                 result[0][i+1][1][0] == "Ch 2 " or result[0][i+1][1][0] == "ch 2 ":
                dict2[res[1][0]] = result[0][i+2][1][0]


def OCR_dleta(imagepath, dict3):
    print(f'\n{imagepath} is OCRing...')
    img = cv2.imread(imagepath)
    # OCR results
    result = ocr_dleta.ocr(img, cls=False)
    for i, res in enumerate(result[0]):
        print(res)
        if "setuptime" in imagepath or "holdtime" in imagepath:
            match = re.match(r"(△t: )([-]?[0-9.]+) (ns|μs|ms|s)", res[1][0])
            if match and "setuptime" in imagepath:
                dict3["setuptime"] = res[1][0]
            elif match and "holdtime" in imagepath:
                dict3["holdtime"] = res[1][0]
...
def Create_New_Excel(root):
    ...
    newwb.save(os.path.join(root,'Project_Signal-Integrity(I2C_C)_EVT_Test_Report_Rev.A0.xlsx')) # name the excel "Project_Signal-Integrity_EVT_Test_Report_Rev.A0.xlsx"
...
def UnitTransform(dict):
    ...
    v = ["Top", " Top", "Top "]
    mv = ["Base", " Base", "Base "]
    ns = ["Rise Time", "Fall Time", "holdtime", "setuptime", " Rise Time", " Fall Time", " Hold Time", " Setup Time", "Rise Time ", "Fall Time ", "Hold Time ", "Setup Time ",
          "Fal Time", " Fal Time", "Fal Time "]
    us = ["High Time", "Low Time", " High Time", " Low Time", "High Time ", "Low Time "]
    khz = ["Frequency", " Frequency", "Frequency "]

    for key in dict:
        if key == "setuptime" or key == "holdtime":
            dict[key] = dict[key][dict[key].find(": ")+1:]
...
def main():
    ...
    wb = opxl.load_workbook(os.path.join(root,'Project_Signal-Integrity(I2C_C)_EVT_Test_Report_Rev.A0.xlsx'))
    ws = wb.active
    ExcelFormat(wb)
    row = 52
    row = VDDVol(ws, row)
    ...
    for dir in dirs:
        ...
        for file in files:
            if file == "vihvil.png":
                OCR_table(os.path.join(dir,file), CLKdict, DATAdict)
            else:
                OCR_dleta(os.path.join(dir,file), dletadict)

        print("CLK: ", CLKdict)
        print("DATA: ", DATAdict)
        print("DLETA: ", dletadict)
        ...
        UnitTransform(dletadict)

        clk_max = CLKdict["Top"] if "Top" in CLKdict else (CLKdict["Top "] if "Top " in CLKdict else (CLKdict[" Top"] if " Top" in CLKdict else None))
        data_max = DATAdict["Top"] if "Top" in DATAdict else (DATAdict["Top "] if "Top " in DATAdict else (DATAdict[" Top"] if " Top" in DATAdict else None))
        clk_min = CLKdict["Base"] if "Base" in CLKdict else (CLKdict["Base "] if "Base " in CLKdict else (CLKdict[" Base"] if " Base" in CLKdict else None))
        data_min = DATAdict["Base"] if "Base" in DATAdict else (DATAdict["Base "] if "Base " in DATAdict else (DATAdict[" Base"] if " Base" in DATAdict else None))
        ...
        clk_rise = CLKdict["Rise Time"] if "Rise Time" in CLKdict else (CLKdict["Rise Time "] if "Rise Time " in CLKdict else (CLKdict[" Rise Time"] if " Rise Time" in CLKdict else None))
        data_rise = DATAdict["Rise Time"] if "Rise Time" in DATAdict else (DATAdict["Rise Time "] if "Rise Time " in DATAdict else (DATAdict[" Rise Time"] if " Rise Time" in DATAdict else None))
        clk_fall = CLKdict["Fall Time"] if "Fall Time" in CLKdict else (CLKdict["Fall Time "] if "Fall Time " in CLKdict else (CLKdict[" Fall Time"] if " Fall Time" in CLKdict else\
                   CLKdict["Fal Time"] if "Fal Time" in CLKdict else (CLKdict["Fal Time "] if "Fal Time " in CLKdict else (CLKdict[" Fal Time"] if " Fal Time" in CLKdict else None))))
        data_fall = DATAdict["Fall Time"] if "Fall Time" in DATAdict else (DATAdict["Fall Time "] if "Fall Time " in DATAdict else (DATAdict[" Fall Time"] if " Fall Time" in DATAdict else\
                    DATAdict["Fil Time"] if "Fil Time" in DATAdict else (DATAdict["Fal Time "] if "Fal Time " in DATAdict else (DATAdict[" Fal Time"] if " Fal Time" in DATAdict else None))))
        row1 = row
        ...
        clk_pos = CLKdict["High Time"] if "High Time" in CLKdict else (CLKdict["High Time "] if "High Time " in CLKdict else (CLKdict[" High Time"] if " High Time" in CLKdict else None))
        clk_neg = CLKdict["Low Time"] if "Low Time" in CLKdict else (CLKdict["Low Time "] if "Low Time " in CLKdict else (CLKdict[" Low Time"] if " Low Time" in CLKdict else None))                  
        row1 = row
        ...
        hold= dletadict["holdtime"] if "holdtime" in dletadict else None
        setup = dletadict["setuptime"] if "setuptime" in dletadict else None
        row1 = row
        row = SetupHoldTimmingHeader(ws, row)
        row, misslist = Result_1(ws, row, locname2, locprobe2, setup, "250", hold, "0", "H", [2, 0, 2, 0, 0], misslist)   
        row = NumOfTests(row1, row, ws)        

    wb.save(os.path.join(root,'Project_Signal-Integrity(I2C_C)_EVT_Test_Report_Rev.A0.xlsx'))

    print("total error cell: ", len(misslist))
    print(misslist)

    f = open(os.path.join(root,'fail_to_rec(I2C_C).txt'), 'w')
